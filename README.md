...
Все команды набираются в терминале локального репозитория. Как правило это данный набор команд, но он может быть и другим (в удалённом репозитории показываются подсказки в зависимости от варианта переноса репозитория - достаточно следовать им)
...
## Предложение о внесении измнений в чужой репозиторий

* В нужном чужом репозитории на GitHub нажимается fork, это позволит скопировать чужой репозиторий в удалённый репозиторий в собственном аккаунте.
* в терминале локального репозитория набирается команда git clone (link) удалённый репозиторий копируется в локальный. Ссылка копируется на GitHub в выпадающем меню (Code)
* В локальной версии создаётся новая ветка, в которую вносятся изменения, создаются нужные файлы и коммиты
* После записи и фиксации изменений в локальной версии (git add file_name, git commit) локальный репозиторий с изменениям отправляется на удалённый репозиторий
* В личном удалённом репозитории выбирается опция отпраки запроса на внесение изменений (можно отправить с комментариями или без). Владелец исходного репозитория после получения запроса принимает решение принять изменения или нет.
